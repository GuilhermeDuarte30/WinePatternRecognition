...
######  Varimax rotation
######### most popular rotational method
######### Simpler structure by maximizing variance of loadings within each component across variables, i.e., makes large loadings larger and small loadings smaller within each component
######### It uses orthogonal rotations, i.e. components are kept uncorrelated
######### It spreads the variance from first (largest) factor to other smaller factors
...
#####   The score of the observations is simply the weighted sum of the variable scores, where the weights are the eigenvectors
...
plot(silhouette(groups.k3,demodist2), col=1:3, border=NA)
...
#Silhouette
plot(silhouette(kmeans.k3$cluster,demodist2), col=1:3, border=NA) 
...
wssplot <- function(xx, nc=15, seed=1234){
  ...
  plot(1:nc, wss,main ="Elbow plot", type="b", xlab="Number of Clusters",
